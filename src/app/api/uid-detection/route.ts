...
import { InferenceSession, Tensor, env } from 'onnxruntime-web'; // Import specific named exports
...
// --- Preprocessing ---
async function preprocessImage(imageBuffer: Buffer): Promise<{ tensor: Tensor, originalWidth: number, originalHeight: number }> {
...
function processOutput(outputTensor: Tensor, originalWidth: number, originalHeight: number): DetectionBox[] {
...
function applyNMS(boxes: Array<{ x1: number; y1: number; x2: number; y2: number; score: number; classId: number }>, iouThreshold: number): Array<{ x1: number; y1: number; x2: number; y2: number; score: number; classId: number }> {
...
function calculateIoU(box1: { x1: number; y1: number; x2: number; y2: number }, box2: { x1: number; y1: number; x2: number; y2: number }): number {
...
export async function GET(request: NextRequest) {
    return NextResponse.json({ message: 'Send a POST request with form-data including an "image" field.' });
}